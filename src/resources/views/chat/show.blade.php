...
自分が利き手でも匿名のボタンがある
自分がポストしたとき名前が匿名？
スクロールがローディングして下に行くように --}}

<x-body-common :title="'CHAT DETAIL'">
    <!-- メガホンアイコン -->
    <a href="#top" class="fixed top-4 right-0 flex items-stretch gap-4 bg-white shadow-md p-4 transform origin-center hover:bg-gray-100 hover:scale-105 transition">
        <img src="{{ asset('images/megaphon.svg') }}" alt="Megaphon" class="h-8 w-8">
    </a>

    <!-- 匿名/実名切り替えボタン -->
    <div class="flex justify-end z-30 fixed top-12 right-0">
        <form action="{{ route('chat.toggle-anonymity', $posting->id) }}" method="POST">
            @csrf
            @method('PATCH')
            <button type="submit" class="px-4 py-2 bg-gray-500 text-white rounded transform origin-center hover:bg-gray-600 hover:scale-105 transition">
            ...
            <div
                ...
                @else
                    ...
                    @else
                        ...
                        <div class="bg-gray-300 w-12 h-12 rounded-full flex-none overflow-hidden">
                            <img src="{{ route('user.icon', Auth::user()->icon_image) }}" alt="User Icon" class="object-cover w-12 h-12">
                        </div>
                    @endif
                @endif
            </div>
